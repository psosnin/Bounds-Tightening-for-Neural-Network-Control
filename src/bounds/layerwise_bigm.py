...
def getBounds(params, input_bounds, frac_binary):
    ...
    for l in range(len(params) - 1):
        ...
        hl = addBigMConstr(m, hl_minus_1, W[l], b[l], bounds[-1], int(b[l].size * fb))
        ML, MU = tighten(m, W[l+1] @ hl + b[l+1])
        bounds.append((ML, MU))
    return bounds


def getMultiStepBounds(params, input_bounds, A, B, K, relax=False):
    """
    ...
    by solving layerwise MIP/LPs using the bigM formulation.
    Parameters:
        params: list of tuples (W, b) where W is the weight matrix and b is the bias vector for each layer
        input_bounds: tuple (L, U) where L is the lower bound and U is the upper bound on the input
    ...
    for k in range(K):
        ...
        hidden_bounds = getBounds(params, next_state_bounds, fb)
        ...
        hL_minus_1 = addBigMConstr(m, hL_minus_2, WL_minus_1, bL_minus_1, hidden_bounds[-2], int(fb * bL_minus_1.size))
