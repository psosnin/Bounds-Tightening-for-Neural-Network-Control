...
run = wandb.init(
    ...
    entity=ENTITY,
    mode=MODE,
    job_type=MULTI_STEP_BOUNDS_JOB_TYPE,
    name=MULTI_STEP_BOUNDS_JOB_NAME,
...
for bound_name, bound_fn in MULTI_STEP_BOUNDS_METHODS.items():
    start = time.time()
    bound = bound_fn(params, x_in_bounds, A, B, K_STEPS)

    # calculate metrics on bounds
    total_time = time.time() - start
    mean_final_layer_width = np.mean(bound[-1][-1][1] - bound[-1][-1][0])
    max_final_layer_width = np.max(bound[-1][-1][1] - bound[-1][-1][0])

    table.add_data(bound_name, total_time, mean_final_layer_width, max_final_layer_width)

    artifact = wandb.Artifact(
        name=MULTI_STEP_BOUND_ARTIFACT_NAME.format(bound_name=bound_name, network_type=network_type, seed=seed),
        type=MULTI_STEP_BOUND_ARTIFACT_TYPE,
        metadata=dict(config)
    )

    with artifact.new_file(MULTI_STEP_BOUND_FILENAME, mode="wb") as file:
        np.save(file, np.array(bound, dtype=object))

    run.log_artifact(artifact)
    run.log({"multi_bounds_table": copy(table)})
